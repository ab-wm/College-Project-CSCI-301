...
public class Statistics {
	...
	public Statistics() {
		...
		this.log = fileSeparator + "statistics_Normal.log"; // Change Task 3: Changed default file to maintain consistency
		System.out.println("DEBUG USER HOME: " + path + log);
		readStatistics();
	}

	// Change Task 3: Parameterized constructor to create custom files for desired game modes
	public Statistics(String gameMode) {
		this.wordsGuessed = new ArrayList<>();
		String fileSeparator = System.getProperty("file.separator");
		this.path = System.getProperty("user.home") + fileSeparator + "Wordle";
		this.log = fileSeparator + "statistics_" + gameMode + ".log"; // File for each game mode
		System.out.println("DEBUG USER HOME: " + path + log);
		readStatistics();
	}
