...
public class WordleModel {
    ...
    // Change Task 3: Parameterized constructor to control the number of rows and columns
    // Constructor is the same as the default, expect for being able to set the number of row, columns
    // and difficultly.
    public WordleModel(int columnCount, int maximumRows, String gameMode) {
        this.currentColumn = -1;
        this.currentRow = 0;
        ...
        this.wordleGrid = initializeWordleGrid();
        this.guess = new char[columnCount];
        this.statistics = new Statistics(gameMode);
    }
    ...
    // Change Task 1: Method to return wordlist
    public List<String> getWordList() {
        return wordList;
    }
    // Change Task 1: Method to return guess
    public char[] getGuess() {
        return guess;
    }

    // Change Task 2: Method to check if the user has run out of guesses
    public boolean moreGuesses() {
        return currentRow - 1 < maximumRows; // Adjustment for the last 6th row
    }
}
