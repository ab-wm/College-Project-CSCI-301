...
public class WordleFrame {
	...
	private JMenuBar createMenuBar() {
		...
		// Change Task 3: Added another menu bar item for difficulty
		// Create menu item
		JMenu difficultyMenu = new JMenu("Difficulty");
		menuBar.add(difficultyMenu);
		// Add Normal difficultly option
		JMenuItem normalDifficulty = new JMenuItem("Normal");
		normalDifficulty.addActionListener(event -> difficultyRestart("Normal"));
		difficultyMenu.add(normalDifficulty);
		// Add Hard difficultly option
		JMenuItem proDifficulty = new JMenuItem("Pro");
		proDifficulty.addActionListener(event -> difficultyRestart("Pro"));
		difficultyMenu.add(proDifficulty);
	...
	// Change Task 3: Method to restart the application if difficultly is changed
	// Works by writing statistics of the current level of play, trashing the present window
	// and then recreates it by calling a new model of the desired difficulty
	public void difficultyRestart (String difficulty) {
		model.getStatistics().writeStatistics();
		frame.dispose();
		switch (difficulty) {
			case "Normal":
				new WordleFrame(new WordleModel(5,6, "Normal"));
				break;
			case "Pro":
				new WordleFrame(new WordleModel(6,7, "Pro"));
				break;
		}
	}
