...
import java.util.List; // Change Task 1: Additional import statement to handle wordlist
...
public class KeyboardButtonAction extends AbstractAction {
	...
	public void actionPerformed(ActionEvent event) {
		...
		case "Enter":
			if (model.getCurrentColumn() >= (model.getColumnCount() - 1)) {
				// Change Task 1: Check to see if the inputted word is a valid word
				// 		          If the word is not valid, ignore the request
				if (checkValidGuess(model.getGuess())) {
	...
	// Change Task 1: Method to check if the guess is valid by comparing to the word list
	private boolean checkValidGuess(char[] guessArray) {
		// Create string for guess and set up wordlist to check input
		String guess = "";
		List<String> wordList = model.getWordList();
		// Build the guess string from char array
		for (char c : guessArray)
			guess += c;
		// Check if the word exists in the list. Return false if not found
		for (String testWord : wordList ) {
			if (guess.equals(testWord.toUpperCase()))
				return true;
		}
		return false;
	}
