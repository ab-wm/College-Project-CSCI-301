...
public class StatisticsDialog extends JDialog {
	...
	// Change Task 2: Modified method to create a panel for displaying the secret word
	// Additional Change: Added logic to check if the user guessed the word or failed to do so for the message
	private JPanel createTopPanel() {
		JPanel panel = new JPanel(new BorderLayout());
		panel.setBorder(BorderFactory.createEmptyBorder(0, 5, 5, 5));
		
		panel.add(createTitlePanel(), BorderLayout.NORTH);
		if (model.getStatistics().getCurrentStreak() == 0) // Failure
			panel.add(createSecretWordPanel("Failed to guess the secret word in " + model.getMaximumRows() + " guesses:"));
		else // Success
			panel.add(createSecretWordPanel("Secret Word:"));
	...
	// Change Task 2: Create new panel to display the current word below the title
	// Additional Change: Custom messages when providing the guess word
	private JPanel createSecretWordPanel(String message) {
		JPanel panel = new JPanel(new FlowLayout());
		panel.setBorder(BorderFactory.createEmptyBorder(0, 5, 17, 5));
		// Secret word titling panel
		JLabel secretWordLabel = new JLabel(message);
		secretWordLabel.setFont(AppFonts.getSubtitleFont());
		panel.add(secretWordLabel);
		// Secret word panel
		JLabel currentWordLabel = new JLabel(model.getCurrentWord());
		currentWordLabel.setFont(AppFonts.getSubtitleFont());
